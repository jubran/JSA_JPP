...
```
**Steps 4-6 are the Issuer's own isolation work, done alone, before the permit is issued** (per "Generation Work Permit Procedure Rev-2" §4.1.1.5-4.1.1.8: the Issuer "carries out all isolations... locking and tagging"). **Steps 9-10 are the Receiver's own field check, done after the permit and TBT, right before they touch the equipment**: the Receiver physically verifies the lock/tag are in place (per §4.1.1.12-4.1.1.13: "it is the responsibility of the receivers to verify... that all isolations and grounding are actually performed in the field and locked out/tagged out") and only then applies their own Personal Lock. This is a real field check at the equipment, done separately from and after the Issuer's steps 4-6 — never merged into them, and never skipped just because the Receiver was present during the walk-down (step 1). Do not merge steps 4-6 into one step, do not merge steps 9-10 into one step, and never merge the Issuer's isolation lock/tag (step 6) with the Receiver's Personal Lock (step 10) — these are separate Job Steps done by different people at different points in the sequence.
...
- All energy sources are identified (step 1)
...
- Zero Energy / Depressurization / no-leakage verification (step 5) matches the isolation type(s) actually used, and is **Permit Issuer only** — not joint, not Receiver
...
- **Issuer-only actions (steps 4-6) are never written as something the Receiver does.** Step 6 ("Apply isolation lock/tag") is Responsible: Permit Issuer only — never Permit Receiver, and never merged with the Receiver's own Personal Lock step.
- **Receiver's field-check step (step 9) is present as its own Job Step**, worded "Verify boundary tag and isolation lock are applied", Responsible: Permit Receiver, placed after the permit (step 7) and TBT (step 8) — not folded into any Issuer step, and not omitted.
- **Electrical/Mechanical boundary — zero mention, not just no detail:** for every equipment group except **88TK and 88PF**, this JSA is Electrical's document only. Coupling, alignment, and belt work are entirely Mechanical's job and Mechanical's own JSA — **do not add any Job Step, hazard, cause, control, or reference that names coupling, alignment, or the Mechanical department in any form**, including a hand-off, confirmation, or "wait for Mechanical to finish" step. Electrical may not know whether or when Mechanical is even working on this equipment at the same time, so no such step is written. Electrical's own step sequence simply runs: disconnect electrical cable terminals → (Mechanical's coupling work happens outside this JSA, unmentioned) → reconnect electrical cable terminals → rotation/functional test → final inspection and housekeeping → remove personal lock → close PTW. Use exactly this wording — "Disconnect electrical cable terminals" / "Reconnect electrical cable terminals" — for these two Job Steps, never a looser phrase like "Disconnect/Reconnect electrical services". Only for 88TK and 88PF does the coupling/alignment work itself appear as Electrical Job Steps, since Electrical performs it directly there.
...
- **Total Damage vs Equipment Damage (2026-09-25 final):** these are two different Consequences, not interchangeable. `Total Damage` = the equipment is destroyed beyond repair and scrapped (e.g. a severe drop, major impact). `Equipment Damage` = partial, repairable damage (e.g. a minor drop within a controlled lift, or a short circuit from a wrong connection) — the equipment remains fixable. Pick the one the specific step's credible scenario actually supports; do not default to "Equipment damage" for every consequence.
...
- **Fixed residual C values (2026-09-25 final, overrides any other guidance above for these specific steps):**
  - `Disconnect electrical cable terminals` — **C4**
  - `Reconnect electrical cable terminals` — **C3** (lower than disconnect: isolation/lock are still in place at this point)
  - Crane or Forklift lift/lower/remove/install of the motor — **C3** for the equipment-drop scenario (break/malfunction from a fall during the lift), unless that same step also has a separate credible direct human-exposure path, which is scored on its own
- After correct isolation + voltage test + personal lock + no remaining live exposure, residual is never Fatality
- Lifting operations at JEPP never use Fatality/C5 — max severity is C4 where the lift crew has real exposure; the equipment-drop consequence itself is C3 per the fixed value above
...
- **Issuer-only actions are never written as this JSA's controls performed by the Receiver.** "Apply isolation lock/tag" (step 6) is the Issuer's own separate Job Step with Detail bullet `Isolation Lock/Tag`, Responsible: Permit Issuer only. "Verify boundary tag and isolation lock are applied" (step 9) is the Receiver's own separate Job Step with Detail bullet `Lock/Tag Verification`, Responsible: Permit Receiver only, followed by its own "Apply Personal Lock" step (step 10) with Detail bullet `Personal Lock`. These are four distinct Job Steps (5, 6, 9, 10) — never collapsed into one, and never cross-attributed (the Receiver never "applies" the Issuer's lock/tag, and the Issuer never "verifies" or applies the Receiver's Personal Lock).
...
**Zero mention, not just the Responsible column:** this restriction is not limited to who's listed as Responsible — for a Forklift-only or Overhead-Crane-only step, "Rigger" and "Signal Man" must not appear anywhere in that step's row at all: not as a `Detail` control-measure bullet (e.g. never write "Signal Man Control", "Signal Man", or "Rigger Verification" as a control name for a Forklift/Overhead Crane step), not in `Potential Hazard`, `Cause`, or `Consequences` text, and not as Responsible. If a control bullet naming a person/role as the "control" is tempting (e.g. wanting to note that someone directs the forklift), name the actual control being performed instead (e.g. `Forklift Route Check`, `Exclusion Zone`) rather than naming a role that isn't allowed on that step. Check every Forklift-only and Overhead-Crane-only step's full row, not just its Responsible cell, for a stray Rigger/Signal Man mention.
...
3. Sequence logic is correct and matches the Register's `Task Scope / Activities` order
...
11. Roles are correct, only from the allowed list, and match the Register's `Lifting Equipment` field; Rigger/Signal Man appear only on steps that actually use a **Mobile Crane** — never on a Forklift-only step, and never on an Overhead Crane step either, and never anywhere in that step's row (Detail/Hazard/Cause/Consequences included, not just Responsible)
...
28. No step has the Permit Receiver "applying" the boundary tag or isolation/red lock as their own action — that is the Issuer's own step (6); the Receiver's separate steps are "Verify boundary tag and isolation lock are applied" (9) then "Apply Personal Lock" (10)
34. The 10-step opening sequence (Stage 2) is present in the exact order — site walk-down, equipment/tools/PPE check, process isolation in system, isolate energy source (Issuer), verify zero voltage (Issuer only), apply isolation lock/tag (Issuer only), Obtain PTW, TBT, Verify boundary tag/lock applied (Receiver), Apply Personal Lock (Receiver) — and `Obtain Work Permit (PTW)` is never the first Job Step, and TBT never comes before the permit
35. The standalone "Verify boundary tag and isolation lock are applied" step (9) IS present, worded exactly that way, Responsible: Permit Receiver, placed after TBT — it must never be missing and never merged into another step
...
TBT placed before the work permit is obtained                   → FAIL
Standalone "Verify boundary tag and isolation lock" step missing → FAIL
"Verify zero voltage" Responsible includes Permit Receiver       → FAIL
```
...
## Addendum — 2026-09-25 cross-file consistency pass (SKILL.md / General_Instructions.md / JPPMDES.xlsx)

This pass confirmed the 10-step isolation opening above (already final in this file) had not yet propagated to `SKILL.md` (still said "Work permit + SIMOPS confirmation is the first step") or to `General_Instructions.md` Part 1 §9-b, and — critically — the actual `Task Scope / Activities` narrative in `JPPMDES.xlsx` Tasks Register still opened every isolation task with the old "Obtain PTW and confirm SIMOPS; identify energy sources; apply electrical isolation..." order. All three were corrected to this file's 10-step rule; see the automated conflict report delivered alongside this file for the full before/after.

**Resolved in this pass:**
- **Task Name voltage naming (24 rows, 8 equipment groups: 88FC, 88QV, 88TK, 88WC, 88PF, 88AB, 88QA, 88TM):** `Task Name` said "480 VAC" while `Voltage`, `Job Description`, and `Area, Equipment ID` for the same rows already said "460 VAC" (the 2026-09-23 nameplate-data correction had updated those three fields but not `Task Name`). Resolution: `Task Name` corrected to 460 VAC to match. Rationale — not a guess: `Equipment Notes & Rules` confirms MCC1-AC/MCC2-AC bus/breaker rating is 480V while the motor nameplate for these 8 groups is 460V; both figures are simultaneously correct in their own context, and this project's own dated note (`General_Instructions.md`, "هذا الجدول محدَّث بالكامل بتاريخ 2026-09-25") already fixed the group-reference table to 460V for these groups — `Task Name` was simply the one field the fix missed. 88KA, 88HT (480V breakers), 88HQ (480V nameplate), CRUDE FWD/DIESEL FWD (480V nameplate), and 88QE (125 VDC) needed no change — no 120 VDC remains anywhere in the file.
- **Isolation step-order propagation (31 rows):** every `Isolation Required = Yes` task's `Task Scope / Activities` opening rewritten to the 10-step order (Stage 2), preserving each row's task-specific breaker/cable notes and the rest of its scope text unchanged.
- **Bearing tasks:** confirmed exactly 5 (T043–T047: 88WC, 88TK, CRUDE FWD, DIESEL FWD, 88HQ), each with weight/voltage/lifting equipment matching the group reference table and the corrected 10-step sequence.
- **Coupling/Mechanical boundary:** confirmed zero leakage outside the 88TK/88PF exception (T007/T008 88TK, T016/T017 88PF are the only rows with coupling open/close text; T044's 88TK bearing task correctly mentions coupling disconnect/reconnect as its own 88TK exception).
- **Consequence wording:** confirmed the register and both instruction files use plain "Equipment damage" throughout — no "Major"/"Total damage" inflation found.
- **MAINTAIN/REPLACE/INSPECT methodology:** confirmed internally consistent across `Tasks Notes & Assumptions` and the per-row data — no changes needed.

**UNRESOLVED (none material found in this pass).** If a future equipment group is added to `JPPMDES.xlsx` without a matching row in the `General_Instructions.md` group-reference table, that gap must be flagged the same way — never assumed from a neighboring group.
